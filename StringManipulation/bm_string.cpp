...
// Find the last occurence of subsequence(pattern[j,j+k-1])
// that match the good suffix(pattern[size-k,size-1]).
// suffixes' index(k) is the length of the good-suffix(pattern[size-k, size-1]).
// suffixes' value(j) is the begin index of
// the last occurred subsequence(pattern[j,j+k-1]).
void good_suffix_shift(const char pattern[], int size, int suffixes[],
                       bool prefix[]) {
    for (int i = 0; i < size; ++i) {
        suffixes[i] = -1;
        prefix[i] = false;
    ...
    for (int i = 0; i < size - 1; ++i) {
        // For every i, we find the overlap suffix size between pattern[0, i]
        // and pattern[0, size-1]
        // Find the greatest overlap and its begin index in the pattern.
        int begin_index = i;
        int overlap_size = 1;
        while (begin_index >= 0 &&
               pattern[begin_index] == pattern[size - overlap_size]) {
...
// Return move step number
int shift_by_good_suffix(int bad_char_index, int pattern_size,
                         int suffix[], bool prefix[]) {
    int suffix_length = pattern_size - (bad_char_index + 1);
    if (suffix[suffix_length] != -1) {
        return (bad_char_index + 1) - suffix[suffix_length];
    }
    for (int suffix_sub_start = bad_char_index + 2;
         suffix_sub_start <= pattern_size - 1; ++suffix_sub_start) {
        if (prefix[pattern_size - suffix_sub_start]) {
            // steps: suffix_sub_start - suffix[pattern_size - suffix_sub_start]
...
vector<int> boyer_moore(const char target[], int target_size,
    ...
    while (i <= target_size - pattern_size) {
        ...
        i = i + (x > y ? x : y);
        cout << "bad char move steps=" << x << ","
             << "good suffix move steps=" << y << endl;
    }
    return positions;
}
